...
function getAccessToken(cb){
    ...
    client.get(config.ding.prefix+"access_token",function(err,keys) {
        ...
        if (_.has(key, "access_token") && now - key.expire_time < 7200) {
            //获取到，并且未过期
            //返回token
            cb(null,key);
        }else{
...
function tryGetTicket(token,url,cb){
    ...
    async.waterfall([
        ...
        if(err){
            ...
            var client = redis.createClient(config.redis.port,config.redis.host,config.redis.auth);
            client.set(config.ding.prefix +"ticket",'{"ticket":"'+dingTicket.ticket+'","expire_time":'+dingTicket.time+'}',redis.print);
            client.quit();//必须关闭
            var str = 'jsapi_ticket=' + dingTicket.ticket + '&noncestr=' + noncestr + '&timestamp=' + timestamp + '&url=' + url;
            console.log(str);
...
function getJsTicket(token,url,cb){
    ...
    client.get(config.ding.prefix+"ticket",function(err,keys){
        client.quit();
        var key = JSON.parse(keys);
        if (_.has(key, "ticket") && now - key.expire_time < 7200) {
            //获取到，并且未过期
            //返回token
            cb(null,key);
        }else{
...
function tryGetChannelTicket(token,url,cb){
    ...
    async.waterfall([
        ...
        if(err){
            ...
            var client = redis.createClient(config.redis.port,config.redis.host,config.redis.auth);
            client.set(config.ding.prefix +"channel_ticket",'{"ticket":"'+dingChannelTicket.ticket+'","expire_time":'+dingChannelTicket.time+'}',redis.print);
            client.quit();//必须关闭
            var str = 'jsapi_ticket=' + dingChannelTicket.ticket + '&noncestr=' + noncestr + '&timestamp=' + timestamp + '&url=' + url;
            var signature = crypto.createHash('sha1').update(str).digest('hex');
            cb(null, {
                corpId: config.ding.corpId,
                timeStamp: timestamp,
...
function getJsChannelTicket(token,url,cb){
    ...
    client.get(config.ding.prefix+"channel_ticket",function(err,keys){
        client.quit();
        var key = JSON.parse(keys);
        if (_.has(key, "channel_ticket") && now - key.expire_time < 7200) {
            //获取到，并且未过期
            //返回ticket
            cb(null,key);
        }else{
